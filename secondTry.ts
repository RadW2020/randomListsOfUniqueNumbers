...
function findDuplicates(arr: string[]) {
  ...
  return newNewArr.size !== newArr.length;
}

function findOccurrences(arr: string[]) {
    const veces3 = arr.filter(x => x.substring(7, 8) == '3').length
    const veces2 = arr.filter(x => x.substring(7, 8) == '2').length
    const veces1 = arr.filter(x => x.substring(7, 8) == '1').length
    console.log(`[${veces1},  ${veces2},  ${veces3}]`)
  }
...
function aleatorFlojo(arr1: string[], arr2: string[]) {
  arr1.forEach((a1, i) => {
    let continuar = true;
    arr2.forEach((a2, j) => {
      if (Math.random() >= 0.3 && continuar == true) {
        const old1 = a1;
        const old2 = a2;
        arr1[i] = old2;
        arr2[j] = old1;
...
listas.forEach((x, i) => {
  findOccurrences(x);
  if (findDuplicates(x)) {
    console.log(`ERROR: hay duplicados en la lista${i}`
    )
  }
});
